    ...
    <Link to={`/anime/${media.id}`} className="group relative cursor-pointer block">
      ...
      <div className="relative aspect-[2/3] rounded-md overflow-hidden bg-gray-200 mb-2 shadow-sm border border-black/5 dark:border-white/5">
        ...
        <div className="absolute top-2 right-2 flex flex-col items-end gap-1">
          <div className="px-1.5 py-0.5 bg-brand text-white text-[10px] font-black rounded shadow-lg backdrop-blur-sm">
            {Math.round(media.score)}%
          </div>
          <div className="px-1.5 py-0.5 bg-black/60 text-white text-[8px] font-black rounded backdrop-blur-sm uppercase tracking-widest border border-white/10">
            {media.type}
          </div>
        </div>
      </div>
