...
export const UpdateNotification: React.FC = () => {
  const [update, setUpdate] = useState<{ latestSha: string; message: string; date: string } | null>(null);
  const [isVisible, setIsVisible] = useState(false);

  useEffect(() => {
    const unsubscribe = updateService.subscribe((info) => {
      if (info?.hasUpdate) {
        setUpdate({ latestSha: info.latestSha, message: info.message, date: info.date });
        setIsVisible(true);
      }
    });
    return () => unsubscribe();
