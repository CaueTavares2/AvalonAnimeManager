...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const loginWithGoogle = async () => {
    try {
      ...
      if (window !== window.parent && (error.code === 'auth/web-storage-unsupported' || error.message?.includes('storage'))) {
        alert("O Firefox bloqueou o acesso ao armazenamento de login. Por favor, abra o aplicativo em uma NOVA GUIA para fazer o login.");
      } else if (error.code === 'auth/popup-closed-by-user') {
        // user closed, ignore
      } else {
        if (window !== window.parent) {
          alert("O login falhou. Se você estiver usando o Firefox ou Safari, tente abrir o site em uma NOVA GUIA para continuar.");
        } else {
